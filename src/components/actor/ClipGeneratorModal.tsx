...
export const ClipGeneratorModal: FC<ClipGeneratorModalProps> = ({
  ...
  // YouTube download via Modal (server-side, bypasses CORS)
  const triggerYouTubeDownload = useAction(api.r2.triggerYouTubeDownloadToR2);
  ...
  const activeJob = currentYouTubeR2JobId
    ...
    : layout === "meme"
      ? memeJob
      : job;
  const activeResults = layout === "meme" ? memes : clips;
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      try {
        // SERVER-SIDE YOUTUBE DOWNLOAD FLOW
        // Modal downloads via RapidAPI (bypasses CORS), uploads to R2, then processes
        ...
        // Step 2: Trigger Modal to download YouTube video to R2
        // Modal will: download via RapidAPI → upload to R2 → callback to Convex → trigger processing
        const downloadResult = await triggerYouTubeDownload({
          jobId,
        ...
        if (!downloadResult.success) {
          throw new Error(downloadResult.error || "Failed to start YouTube download");
        }

        setYoutubeDownloadProgress(20);
        setIsDownloadingYouTube(false);

        // Step 3: Track the job - Modal will callback when download completes
        ...
        // Progress will continue via real-time job updates from Convex
      } catch (err) {
        setIsDownloadingYouTube(false);
    ...
    [videoUrl, clipCount, layout, slug, actorProfileId, triggerYouTubeDownload, createProcessingJob, isValidYouTubeUrl, durationRange, aspectRatio, clipTone, captionStyle]
  );
  ...
  }, [selectedFile, clipCount, layout, actorProfileId, slug, startUpload, startMemeUpload, durationRange, aspectRatio, clipTone, captionStyle]);
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {status === "idle" && activeUploadState === "idle" && !selectedFile && (
            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Source Type Toggle - YouTube option temporarily disabled
              ...
              */}
              ...
              {/* Submit button - YouTube mode temporarily disabled
              {sourceType === "youtube" && (
                <>
                  <div className="pt-2">
                    <button
          ...
          {status === "completed" && (
            <div className="py-6">
              ...
              {layout !== "meme" && clips && clips.length > 0 && (
                <div className="space-y-2 mb-6 max-h-64 overflow-y-auto">
                  {clips.slice(0, 5).map((clip, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500">
                          Score: {clip.score}% • {Math.round(clip.duration)}s
                        </p>
                      </div>
